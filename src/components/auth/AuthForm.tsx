...
// TODO: ensure change to server (redirect instead of useRouter) for performance and SEO
...
const AuthForm = ({ mode, schema, children, csrfToken }: AuthFormProps) => {
  ...
  const inputClassNames = (field: keyof FieldValues) => {
    let baseClasses =
      "w-full px-4 py-2 bg-white border border-slate-300 rounded-md text-md shadow-sm placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 transition-colors";
    if (errors[field]) {
      baseClasses +=
    ...
    >
      ...
      >
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          {mode !== "resetPassword" && (
            <>
              ...
              />
              {errors.password && (
                <p className="mt-1 pb-1 text-sm text-red-500">
        ...
        <div className="mt-4 flex flex-col space-y-2">
          ...
          {(mode === "signin" || mode === "signup") && (
            <DiscordSignInButton
              handleClick={() => handleSignIn("discord")}
              isLoading={isLoading}
            />
          )}
